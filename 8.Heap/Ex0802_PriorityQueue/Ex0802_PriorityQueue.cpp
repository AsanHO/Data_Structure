...
struct Patient {
    ...
    int time;  // <- 도착시간, 정렬할 때의 2차적인 키(key)로 사용됨, 중증도가
               // 같을 때는 먼저 온 순서
    const char* name;

    // 우선순위 관점에서 봤을 때 a >= b 인지를 반환하는 함수
    friend bool operator>=(const Patient& a, const Patient& b) {
        if (a.severity == b.severity)  // 우선순위가 같을 때는 시간을 기준으로
                                       // 다시 판단 (선착순)
            return a.time <= b.time;   // 숫자로써 time이 작은 것이 먼저 왔다는
                                       // 의미니까 최종 우선순위가 더 높다.
        else
...
int main() {
    ...
    // 증상의 무거운 정도가 동일하다면(severity가 같다면) 먼저 온 환자 먼저
    // 치료(time이 작은 환자 먼저)
