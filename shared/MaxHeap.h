...
class MaxHeap {
   ...
   public:
    ...
    void Pop() {
        ...
        while (child <= size_) {
            // left, right 중에서 더 큰 자식의 인덱스를 찾는다. 이때 자식이
            // 하나라면 찾을 필요 없음
            // TODO:
            child = heap_[child] >= heap_[child + 1] ? child : child + 1;
            // 마지막 값이 더 큰 자식의 값 이상이면 더이상 적절한 위치를 찾을
            // 필요가 없기 때문에 루프 중단
            // TODO:
            if (heap_[child] <= last_item) break;
            // 자식 값을 부모 위치로 복사,
            // TODO:
            heap_[current] = heap_[child];
            cout << "Current = " << current << ", child = " << child << endl;
            Print();
            // 그 자식 위치로 current 인덱스 변경, child 인덱스도 그 다음 자식
            // 위치로 변경
            // TODO:
            current = child;
            child = child * 2;
