...
int main() {
    ...
    // const char infix[] = "8/2+(3+4)*5-1*2";  // Postfix: 8 2 / 3 4 + 5 * + 1 2 * -
    ...
    // 큐에 모두 넣기
    Queue<char> q;
    for (int i = 0; i < size; i++) q.Enqueue(infix[i]);
    ...
    cout << "Evaluated = " << endl << EvalPostfix(postfix) << endl;
...
void InfixToPostfix(Queue<char>& q, Queue<char>& output) {
    ...
    while (!q.IsEmpty()) {
        ...
        {
            while (!s.IsEmpty() && s.Top() != '(' && precedence(s.Top()) >= precedence(c)) {
                output.Enqueue(s.Top());
                s.Pop();
            }
            s.Push(c);
